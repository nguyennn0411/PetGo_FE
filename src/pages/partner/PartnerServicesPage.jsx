...
import { Camera, Copy, FileText, ImagePlus, Loader2, Plus, Save, Scissors, Send, Trash2, X } from 'lucide-react';
    ...
    return (
        <PartnerLayout title="Dịch vụ" subtitle="Tạo/cập nhật dịch vụ qua yêu cầu admin duyệt">
            <div className="space-y-6">
                ...
                {showForm && (
                    <form onSubmit={handleSubmitRequest} className="bg-white rounded-[2rem] border border-gray-100 p-6 shadow-sm space-y-5">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-3">
                                ...
                                <label className={`min-h-36 rounded-[2rem] border-2 border-dashed flex flex-col items-center justify-center text-center p-5 cursor-pointer ${form.photoUrls.length >= MAX_PHOTOS ? 'bg-gray-50 border-gray-100 opacity-60 cursor-not-allowed' : 'bg-orange-50/40 border-orange-100 hover:bg-orange-50'}`}>
                                    ...
                                    <p className="font-black text-gray-800 mt-2">{uploading ? 'Đang upload ảnh...' : 'Bấm để chọn/kéo ảnh mô tả'}</p>
                    ...
                    <section className="bg-white rounded-[2rem] border border-gray-100 p-6 shadow-sm space-y-4">
                        <div className="flex items-center justify-between gap-3">
                            <div>
                                ...
                                <p className="text-sm text-gray-500 font-semibold">Bản nháp có thể sửa/gửi duyệt; yêu cầu chờ duyệt sẽ được admin xem trước.</p>
                            </div>
                            <span className="px-3 py-1 rounded-full bg-gray-100 text-gray-500 font-black text-xs">{requests.length} mục</span>
                        </div>
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            {requests.map((request) => (
                                <div key={request.id} className="rounded-[1.5rem] bg-gray-50 border border-gray-100 p-4 space-y-3">
                                    <div className="flex items-start justify-between gap-3">
                                        <div>
                                            <h3 className="font-black text-gray-900">{request.serviceName || 'Bản nháp chưa đặt tên'}</h3>
                                            <p className="text-xs font-bold text-gray-500">{request.requestType === 'UPDATE' ? 'Cập nhật dịch vụ' : 'Tạo dịch vụ'} · {categoryLabel(request.categories)}</p>
                                        </div>
                                    ...
                                    <div className="flex items-center gap-3 text-sm font-black text-gray-700">
                                        <Camera className="w-4 h-4 text-orange-500" /> {request.photoUrls?.length || 0} ảnh · {request.priceDisplay} / {request.priceUnitLabel}
                                    </div>
                                    {request.adminMessage && <p className="text-sm font-bold text-red-600 bg-red-50 rounded-2xl p-3">Admin: {request.adminMessage}</p>}
                    ...
                    <section className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        {services.map((service) => (
                            <div key={service.id} className="bg-white rounded-[2rem] border border-gray-100 p-6 shadow-sm space-y-4">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    <button onClick={() => startEdit(service)} className="px-4 py-2 rounded-xl bg-gray-900 text-white font-black text-xs">Yêu cầu cập nhật</button>
                                    ...
                                    <button onClick={() => handleToggle(service)} className="px-4 py-2 rounded-xl bg-orange-50 text-orange-600 font-black text-xs">{service.active ? 'Ẩn' : 'Hiển thị'}</button>
                                    <button onClick={() => handleArchive(service)} className="px-4 py-2 rounded-xl bg-red-50 text-red-600 font-black text-xs">Ẩn dịch vụ</button>
                                </div>
                            </div>
                        ))}
                    </section>
